# Pytorch implementation of GRAMGAN model

import torch
import numpy as np

"""
def _get_weight(s, layer_idx, scope, gain=np.sqrt(2), lr_mul=1., eq_lr=True):
    # the init
    fan_in = np.prod(s[:-1]) 
    he_stddev = gain / np.sqrt(fan_in)

    if eq_lr:
        init_stddev = 1. / lr_mul
        rt_coeff = he_stddev * lr_mul
    else:
        init_stddev = he_stddev / lr_mul
        rt_coeff = lr_mul

    initializer = torch.nn.init.normal_(torch.empty(s), 0, init_stddev)

    return initializer * rt_coeff
"""
class _get_weight(torch.nn.Module):
    def __init__(self, shape, layer_idx, scope, gain=np.sqrt(2), lr_mul=1., eq_lr=True):
        super(_get_weight, self).__init__()
        self.shape = shape
        self.layer_idx = layer_idx
        self.scope = scope
        self.gain = gain
        self.lr_mul = lr_mul
        self.eq_lr = eq_lr
        self.init_stddev = eq_lr *(1./lr_mul) + (1-eq_lr)*(gain/(lr_mul*np.sqrt(np.prod(shape[:-1]))))
        self.rt_coeff = (1-eq_lr)*lr_mul + (eq_lr * (gain/lr_mul)) / np.sqrt(np.prod(shape[:-1]))
        self.initializer = torch.nn.Parameter(torch.nn.init.normal_(torch.empty(self.shape), 0, self.init_stddev))

    def forward(self):
        return self.initializer * self.rt_coeff

"""def _bias(x, scope, layer_idx=-1, lr_mul=1., conv=True, vol=False):
    b = torch.nn.init.constant_(torch.empty(x.shape[-1]), 0) * lr_mul
    if conv:
        return x + b.view(1, 1, 1, -1)
    else:
        return x + b """

class _bias(torch.nn.Module):
    def __init__(self, scope, layer_idx=-1, lr_mul=1., conv=True, vol=False):
        super(_bias, self).__init__()
        self.scope = scope
        self.layer_idx = layer_idx
        self.lr_mul = lr_mul
        self.conv = conv
        self.vol = vol
        
    def forward(self,x):
        b = torch.nn.init.constant_(torch.empty(x.shape[-1]), 0) * self.lr_mul
        if self.conv:
            return x + b.view(1, 1, 1, -1)
        else:
            return x + b
  
"""
def downsample(t_in, factor=2):

    #Average pool downsampling
    
    t_out = torch.nn.AvgPool2d([1, factor, factor, 1], [1, factor, factor, 1], padding='VALID',
                           data_format='NHWC')
    return t_out
"""
"""
def conv2d(t_in, n_chan, layer_idx=-1, scope='conv', kernel_size=3, stride=1, gain=np.sqrt(2), lr_mul=1.,
              eq_lr=True):
   #A 2D convolutional layer 
    w = _get_weight([kernel_size, kernel_size, t_in.shape[-1], n_chan], layer_idx, scope, gain=gain,
                    lr_mul=lr_mul, eq_lr=eq_lr)
    return torch.nn.functional.conv2d(t_in, w, stride=stride, padding=1)
"""
class Conv2DLayer(torch.nn.Module):
    def __init__(self, n_chan, layer_idx=-1, scope='conv', kernel_size=3, stride=1, gain=np.sqrt(2),
                 lr_mul=1., eq_lr=True):
        super(Conv2DLayer, self).__init__()
        self.stride = stride
        self.kernel_size = kernel_size
        self.n_chan = n_chan
        self.layer_idx = layer_idx
        self.scope = scope
        self.gain = gain
        self.lr_mul = lr_mul
        self.eq_lr = eq_lr
        
    def forward(self, t_in):
        self.w = _get_weight([self.kernel_size, self.kernel_size, t_in.shape[-1], self.n_chan], self.layer_idx, self.scope,
                                         gain=self.gain, lr_mul=self.lr_mul, eq_lr=self.eq_lr)
    
        return torch.nn.functional.conv2d(t_in, self.w, stride=self.stride, padding=1)
    
class _get_weight(torch.nn.Module):
    def __init__(self, shape, layer_idx, scope, gain=np.sqrt(2), lr_mul=1., eq_lr=True):
        super(_get_weight, self).__init__()
        self.shape = shape
        self.layer_idx = layer_idx
        self.scope = scope
        self.gain = gain
        self.lr_mul = lr_mul
        self.eq_lr = eq_lr
        self.init_stddev = eq_lr *(1./lr_mul) + (1-eq_lr)*(gain/(lr_mul*np.sqrt(np.prod(shape[:-1]))))
        self.rt_coeff = (1-eq_lr)*lr_mul + (eq_lr * (gain/lr_mul)) / np.sqrt(np.prod(shape[:-1]))
        self.initializer = torch.nn.init.normal_(torch.empty(self.shape), 0, self.init_stddev)

    def forward(self):
        return self.initializer * self.rt_coeff
"""
def fc(t_in, n_out, layer_idx=-1, scope='fc', gain=np.sqrt(2), lr_mul=1., eq_lr=True):
    
    # A dense (fully connected) layer
   
    w = _get_weight([t_in.shape[-1] , n_out], layer_idx, scope, gain=gain, lr_mul=lr_mul,
                    eq_lr=eq_lr)
    return torch.matmul(t_in, w)
"""
"""
class fclayer(torch.nn.Module):
    def __init__(self, n_out, layer_idx=-1, scope='fc', gain=np.sqrt(2), lr_mul=1., eq_lr=True):
        super(fclayer, self).__init__()
        self.n_out = n_out
        self.layer_idx = layer_idx
        self.scope = scope
        self.gain = gain
        self.lr_mul = lr_mul
        self.eq_lr = eq_lr
        
    def forward(self, t_in):
        self.w = _get_weight([t_in.shape[-1] , self.n_out], self.layer_idx, self.scope, gain=self.gain, lr_mul=self.lr_mul,
                    eq_lr=self.eq_lr)
        return torch.matmul(t_in, self.w)
"""

def sample_trilinear(t, coords, img_h=128, img_w=128):
    """
    Samples noise octaves in one shot

    :param t: noise cube [n_octaves, noise_res, noise_res, noise_res] (same noise foreach sample in batch)
    :param coords: octave-transformed sampling positions [bs, n_octaves, 3, img_h*img_w]
    :param img_h: height of image to synthesize
    :param img_w: width of image to synthesize
    :return: sampled noise octaves [bs, n_octaves, img_h, img_w]
    """
    # input and output dimensions
    n_octaves, noise_res = t.shape[:2]
    bs = coords.shape[0]

    # all contributing source coordinates (interpolation endpoints)
    x0 = torch.floor(coords[:, :, 0, :])
    x1 = x0 + 1
    y0 = torch.floor(coords[:, :, 1, :])
    y1 = y0 + 1
    z0 = torch.floor(coords[:, :, 2, :])
    z1 = z0 + 1

    # interpolation weights
    w_x = coords[:, :, 0, :] - x0
    w_y = coords[:, :, 1, :] - y0
    w_z = coords[:, :, 2, :] - z0

    # mod for out-of-bound indices
    x0 = torch.fmod(x0, torch.ones_like(x0) * noise_res)
    x1 = torch.fmod(x1, torch.ones_like(x1) * noise_res)
    y0 = torch.fmod(y0, torch.ones_like(y0) * noise_res)
    y1 = torch.fmod(y1, torch.ones_like(y1) * noise_res)
    z0 = torch.fmod(z0, torch.ones_like(z0) * noise_res)
    z1 = torch.fmod(z1, torch.ones_like(z1) * noise_res)

    # for mem efficiency we flatten voxels s.t. we need only one index per element instead of 3
    t = torch.reshape(t, [n_octaves, noise_res**3])
    
    
    # index arrays (in flattened voxel array)
    #idx_x0_y0_z0 = torch.cat([y0 * noise_res + x0 * noise_res**2 + z0], dim=1).long()
    #idx_x0_y0_z1 = torch.cat([y0 * noise_res + x0 * noise_res**2 + z1], dim=1).long()
    #idx_x0_y1_z0 = torch.cat([y1 * noise_res + x0 * noise_res**2 + z0], dim=1).long()
    #idx_x0_y1_z1 = torch.cat([y1 * noise_res + x0 * noise_res**2 + z1], dim=1).long()
    #idx_x1_y0_z0 = torch.cat([y0 * noise_res + x1 * noise_res**2 + z0], dim=1).long()
    #idx_x1_y0_z1 = torch.cat([y0 * noise_res + x1 * noise_res**2 + z1], dim=1).long()
    #idx_x1_y1_z0 = torch.cat([y1 * noise_res + x1 * noise_res**2 + z0], dim=1).long()
    #idx_x1_y1_z1 = torch.cat([y1 * noise_res + x1 * noise_res**2 + z1], dim=1).long()

    
    idx_x0_y0_z0 = (y0 * noise_res + x0 * noise_res**2 + z0).long()
    idx_x0_y0_z1 = (y0 * noise_res + x0 * noise_res**2 + z1).long()
    idx_x0_y1_z0 = (y1 * noise_res + x0 * noise_res**2 + z0).long()
    idx_x0_y1_z1 = (y1 * noise_res + x0 * noise_res**2 + z1).long()
    idx_x1_y0_z0 = (y0 * noise_res + x1 * noise_res**2 + z0).long()
    idx_x1_y0_z1 = (y0 * noise_res + x1 * noise_res**2 + z1).long()
    idx_x1_y1_z0 = (y1 * noise_res + x1 * noise_res**2 + z0).long()
    idx_x1_y1_z1 = (y1 * noise_res + x1 * noise_res**2 + z1).long()

    def _gather(idx):
        out = []
        # print(idx.shape)
        # print(n_octaves)
        # print("t:",t.shape)
        for i in range(n_octaves):
            # print(t[i].shape)
            g = t[i].gather(0, idx[:, i, :].reshape(-1))
            out.append(torch.unsqueeze(g, 1))

        return torch.cat(out, 1)
    
    
    
    # print(idx_x0_y0_z1.shape())
    # print(idx_x0_y1_z0.shape())
    # print(idx_x0_y1_z1.shape())
    # print(idx_x1_y0_z0.shape())
    # print(idx_x1_y0_z1.shape(),idx_x1_y1_z0.shape(),idx_x1_y1_z1.shape())
    
    # gather contributing samples --> now 2D!
    val_x0_y0_z0 = _gather(idx_x0_y0_z0)
    val_x0_y0_z1 = _gather(idx_x0_y0_z1)
    val_x0_y1_z0 = _gather(idx_x0_y1_z0)
    val_x0_y1_z1 = _gather(idx_x0_y1_z1)
    val_x1_y0_z0 = _gather(idx_x1_y0_z0)
    val_x1_y0_z1 = _gather(idx_x1_y0_z1)
    val_x1_y1_z0 = _gather(idx_x1_y1_z0)
    val_x1_y1_z1 = _gather(idx_x1_y1_z1)

    # interpolate along z 
    print(val_x0_y0_z0.shape)
    print(w_z.shape)
    val_x0_y0_z0 * w_z
    print(val_x0_y0_z1.shape)
    print(w_z.shape)
    val_x0_y0_z1 * w_z
    c_00 = val_x0_y0_z0 * (1.0 - w_z) + val_x0_y0_z1 * w_z
    c_01 = val_x0_y1_z0 * (1.0 - w_z) + val_x0_y1_z1 * w_z
    c_10 = val_x1_y0_z0 * (1.0 - w_z) + val_x1_y0_z1 * w_z
    c_11 = val_x1_y1_z0 * (1.0 - w_z) + val_x1_y1_z1 * w_z

    # and interpolate along y 
    c_0 = c_00 * (1.0 - w_y) + c_01 * w_y
    c_1 = c_10 * (1.0 - w_y) + c_11 * w_y

    # and interpolate along x
    c = c_0 * (1.0 - w_x) + c_1 * w_x

    # reshape
    c = c.view(bs, n_octaves, img_h, img_w)

    return c


class add_noise(torch.nn.Module):
    def __init__(self, layer_idx, hidden_dim):
        super(add_noise, self).__init__()
        self.layer_idx = layer_idx
        self.hidden_dim = hidden_dim
        self.per_octave_w16 = torch.nn.Parameter(torch.nn.init.normal_(torch.empty(4, self.hidden_dim), 0, 1))
        self.per_octave_w32 = torch.nn.Parameter(torch.nn.init.normal_(torch.empty(8, self.hidden_dim), 0, 1))
        
    def forward(self, t_in, noise,n_octaves):
        """
        Add noise in 4 layers. In each layer, add 4 (n_octaves=16) or 8 (n_octaves=32) octaves
        :param t_in: pre-activated layer output
        :param noise: noise octaves
        :param layer_idx: layer index
        :return: pre-activated layer output with added noise octaves
        """
        t_out = t_in
        if n_octaves == 16:
            for i in range(4):
                t_out = t_out + noise[:, :, :, self.layer_idx * 4 + i:self.layer_idx * 4 + i + 1] * \
                        torch.reshape(self.per_octave_w16[i, :], [1, 1, 1, self.hidden_dim])
        elif n_octaves == 32:
            for i in range(8):
                t_out = t_out + noise[:, :, :, self.layer_idx * 8 + i:self.layer_idx * 8 + i + 1] * \
                        torch.reshape(self.per_octave_w32[i, :], [1, 1, 1, self.hidden_dim])
        else:
            import sys
            sys.exit("oops, need n_octaves either 16 or 32!")

        return t_out
    


class Sampler(torch.nn.Module):

    """
    MLP that maps 3D coordinate to an rgb texture value
    :param noise_cube: noise, same foreach sample in batch [n_octaves, noise_res, noise_res, noise_res]
    :param coords: xyz coords of slices to synthesize [bs, 3, img_size^2)
    :param transformations: octave transformations [bs, n_octaves, 3, 3]
    :param img_size: training patch size
    :param img_h: height of output image (for inference)
    :param img_w: width of output image (for inference)
    :param act: activation function
    :param scope: scope name
    :param hidden_dim: number of neurons in dense layers
    :param eq_lr: whether to use equalized learning rate
    :return: rgb image
    """

    def __init__(self, img_size=128, img_h=None, img_w=None, act=torch.nn.LeakyReLU, scope='sampler', hidden_dim=128, eq_lr=True):
        super(Sampler, self).__init__()
        self.img_size = img_size
        self.img_h = img_h 
        self.img_w = img_w
        self.act = act
        self.scope = scope
        self.hidden_dim = hidden_dim
        self.eq_lr = eq_lr
        self.base = torch.nn.Parameter(torch.nn.init.constant_(torch.empty(1, 1, 1, hidden_dim), 1))
        #initialize layers
        self.conv0  = Conv2DLayer(hidden_dim, layer_idx=0, kernel_size=1, eq_lr=eq_lr)
        self.add_noise0 = add_noise(0, hidden_dim)
        self.b0 = _bias('conv', layer_idx = 0)

        self.conv1 = Conv2DLayer(hidden_dim, layer_idx=1, kernel_size=1, eq_lr=eq_lr)
        self.add_noise1 = add_noise(1, hidden_dim)
        self.b1 = _bias('conv', layer_idx = 1)

        self.conv2 = Conv2DLayer(hidden_dim, layer_idx=2, kernel_size=1, eq_lr=eq_lr)
        self.add_noise2 = add_noise(2, hidden_dim)
        self.b2 = _bias('conv', layer_idx = 2)

        self.conv3 = Conv2DLayer(hidden_dim, layer_idx=3, kernel_size=1, eq_lr=eq_lr)
        self.add_noise3 = add_noise(3, hidden_dim)
        self.b3 = _bias('conv', layer_idx = 3)

        self.conv4 = Conv2DLayer(hidden_dim, layer_idx=4, kernel_size=1, eq_lr=eq_lr)
        self.b4 = _bias('conv', layer_idx = 4)

        self.conv5 = Conv2DLayer(3, layer_idx=5, kernel_size=1, gain=1., eq_lr=eq_lr)
        self.b5 = _bias('conv', layer_idx = 5)
    


    def forward(self, noise_cube, coords, transformations):
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        if self.img_h is None:
            self.img_h = self.img_size
        if self.img_w is None:
            self.img_w = self.img_size
        
        n_octaves = noise_cube.shape[0]

        # initialize octaves to 2**i for n_octaves = 16 and 2**(i/2) otherwise
        oct_factors = torch.zeros((n_octaves, 3, 3), dtype=torch.float32).to(device)
        for i in range(n_octaves):
            for j in range(3):
                if n_octaves == 16:
                    oct_factors[i, j, j] = 2 ** i
                else:
                    oct_factors[i, j, j] = 2 ** (i/2.)
        
        # Apply octave transformations
        transformations = torch.matmul(transformations, oct_factors)

        # obtain octave sampling coordinates by transforming slice coordinates with octave matrices
        # broadcast (same input coords foreach octave)
        coords = torch.unsqueeze(coords, 1)
        coords = coords.repeat(1, n_octaves, 1, 1)
        # now we have a distinct set of coordinates foreach batch and octave
        octave_coords = torch.matmul(transformations, coords)
        
        # sample noise
        print(noise_cube.shape)
        print(octave_coords.shape)
        print(self.img_h)
        print(self.img_w)
        sampled_noise = sample_trilinear(noise_cube, octave_coords, img_h=self.img_h, img_w=self.img_w)
        # noise in last dimension (NHWC)
        sampled_noise = sampled_noise.permute(0, 2, 3, 1)

        bs = coords.shape[0]
        base = self.base.repeat(bs, self.img_h, self.img_w, 1)

        #trainable layers
        l0 = self.conv0(base)
        l0 = self.add_noise0(l0, sampled_noise, n_octaves)
        l0 = self.b0(l0)
        l0 = self.act(l0)

        l1 = self.conv1(l0)
        l1 = self.add_noise1(l1, sampled_noise, n_octaves)
        l1 = self.b1(l1)
        l1 = self.act(l1)

        l2 = self.conv2(l1)
        l2 = self.add_noise2(l2, sampled_noise, n_octaves)
        l2 = self.b2(l2)
        l2 = self.act(l2)

        l3 = self.conv3(l2)
        l3 = self.add_noise3(l3, sampled_noise, n_octaves)
        l3 = self.b3(l3)
        l3 = self.act(l3)

        # no noise in final hidden layer
        l4 = self.conv4(l3)
        l4 = self.b4(l4)
        l4 = self.act(l4)

        # 2rgb linear
        rgb = self.conv5(l4)
        rgb = self.b5(rgb)

        return rgb

        





"""
def sampler_single(noise_cube, coords, transformations, img_size=128, img_h=None, img_w=None,
                           act=torch.nn.LeakyReLU, scope='sampler', hidden_dim=128, eq_lr=True):
   
    def _add_noise(t_in, noise, layer_idx):
      

        t_out = t_in
        if n_octaves == 16:
            per_octave_w = torch.nn.init.normal_(torch.empty(4, hidden_dim), 0, 1)
            for i in range(4):
                t_out = t_out + noise[:, :, :, layer_idx * 4 + i:layer_idx * 4 + i + 1] * \
                        torch.reshape(per_octave_w[i, :], [1, 1, 1, hidden_dim])
        elif n_octaves == 32:
            per_octave_w = torch.nn.init.normal_(torch.empty(8, hidden_dim), 0, 1)
            for i in range(8):
                t_out = t_out + noise[:, :, :, layer_idx * 8 + i:layer_idx * 8 + i + 1] * \
                        torch.reshape(per_octave_w[i, :], [1, 1, 1, hidden_dim])
        else:
            import sys
            sys.exit("oops, need n_octaves either 16 or 32!")

        return t_out
    
    def post_conv(t_in, noise, layer_idx=-1):
        
        #Adds octave noise and bias, applies activation function
        
        t_out = _add_noise(t_in, noise, layer_idx)
        t_out = _bias(t_out, 'conv', layer_idx)
        t_out = act(t_out)

        return t_out
    
    if img_h is None:
        img_h = img_size
    if img_w is None:
        img_w = img_size
    
    n_octaves = noise_cube.shape[0]

    # initialize octaves to 2**i for n_octaves = 16 and 2**(i/2) otherwise
    oct_factors = np.zeros((n_octaves, 3, 3), dtype=np.float32)
    for i in range(n_octaves):
        for j in range(3):
            if n_octaves == 16:
                oct_factors[i, j, j] = 2 ** i
            else:
                oct_factors[i, j, j] = 2 ** (i/2.)
    
    oct_factors = torch.from_numpy(oct_factors)
    transformations = torch.matmul(transformations, oct_factors)

    # obtain octave sampling coordinates by transforming slice coordinates with octave matrices
    # broadcast (same input coords foreach octave)
    coords = torch.unsqueeze(coords, 1)
    coords = coords.repeat(1, n_octaves, 1, 1)
    # now we have a distinct set of coordinates foreach batch and octave
    octave_coords = torch.matmul(transformations, coords)

    # sample noise
    sampled_noise = sample_trilinear(noise_cube, octave_coords, img_h=img_h, img_w=img_w)
    # noise in last dimension (NHWC)
    sampled_noise = sampled_noise.permute(0, 2, 3, 1)

    with torch.no_grad():
        # start with a learned constant
        base = torch.nn.init.constant_(torch.empty(1, 1, 1, hidden_dim), 1)
        # broadcast (constant per pixel and per sample)
        bs = coords.shape[0]
        base = base.repeat(bs, img_h, img_w, 1)

        layer_idx = 0
        l0 = conv2d(base, hidden_dim, layer_idx=layer_idx, kernel_size=1, eq_lr=eq_lr)
        l0 = post_conv(l0, sampled_noise, layer_idx=layer_idx)

        layer_idx = 1
        l1 = conv2d(l0, hidden_dim, layer_idx=layer_idx, kernel_size=1, eq_lr=eq_lr)
        l1 = post_conv(l1, sampled_noise, layer_idx=layer_idx)

        layer_idx = 2
        l2 = conv2d(l1, hidden_dim, layer_idx=layer_idx, kernel_size=1, eq_lr=eq_lr)
        l2 = post_conv(l2, sampled_noise, layer_idx=layer_idx)

        layer_idx = 3
        l3 = conv2d(l2, hidden_dim, layer_idx=layer_idx, kernel_size=1, eq_lr=eq_lr)
        l3 = post_conv(l3, sampled_noise, layer_idx=layer_idx)

        # no noise in final hidden layer
        layer_idx = 4
        l4 = conv2d(l3, hidden_dim, layer_idx=layer_idx, kernel_size=1,  eq_lr=eq_lr)
        l4 = _bias(l4, 'conv', layer_idx)
        l4 = act(l4)

        # 2rgb linear
        layer_idx = 5
        rgb = conv2d(l4, 3, layer_idx=layer_idx, kernel_size=1, gain=1., eq_lr=eq_lr)
        rgb = _bias(rgb, 'conv', layer_idx)

        return rgb

"""










 
class Mdiscriminator(torch.nn.Module):
    def __init__(self, img_size=128, act=torch.nn.LeakyReLU, scope='mdiscriminator', eq_lr=True):
        super(Mdiscriminator, self).__init__()
        self.img_size = img_size
        self.scope = scope
        self.eq_lr = eq_lr
        self.act = act

        #Average pool downsampling layer
        self.downsample = torch.nn.AvgPool2d([1, 2, 2, 1], [1, 2, 2, 1], padding=1)
        #initialize layers
        self.conv0 = torch.nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1)
        self.b0 = _bias('conv', layer_idx = 0)

        self.conv1 = torch.nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1)
        self.b1 = _bias('conv', layer_idx = 1)

        self.conv2 = torch.nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1)
        self.b2 = _bias('conv', layer_idx = 2)

        self.conv3 = torch.nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1)
        self.b3 = _bias('conv', layer_idx = 3)

        self.conv4 = torch.nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1)
        self.b4 = _bias('conv', layer_idx = 4)

        self.conv5 = torch.nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=1)
        self.b5 = _bias('conv', layer_idx = 5)

        self.conv6 = torch.nn.Conv2d(512, 512, kernel_size=3, stride=2, padding=1)
        self.b6 = _bias('conv', layer_idx = 6)

        self.fc = torch.nn.Linear(512, 512)
        self.b7 = _bias('fc', layer_idx = 7, conv=False)

        self.fc2 = torch.nn.Linear(512, 1)
        self.b8 = _bias('fc', layer_idx = 8, conv=False)

    
    def forward(self, img):
        # conv layer 0 [bs, 128, 128, 32]
        l0 = self.conv0(img)
        l0 = self.b0(l0)
        l0 = self.act(l0)

        # conv layer 1 [bs, 64, 64, 64]
        l1 = self.downsample(l0)
        l1 = self.conv1(l1)
        l1 = self.b1(l1)
        l1 = self.act(l1)

        # conv layer 2 [bs, 32, 32, 128]
        l2 = self.downsample(l1)
        l2 = self.conv2(l2)
        l2 = self.b2(l2)
        l2 = self.act(l2)

        # conv layer 3 [bs, 16, 16, 256]
        l3 = self.downsample(l2)
        l3 = self.conv3(l3)
        l3 = self.b3(l3)
        l3 = self.act(l3)

        # conv layer 4 [bs, 8, 8, 256]
        l4 = self.downsample(l3)
        l4 = self.conv4(l4)
        l4 = self.b4(l4)
        l4 = self.act(l4)

        # conv layer 5 [bs, 4, 4, 512]
        l5 = self.downsample(l4)
        l5 = self.conv5(l5)
        l5 = self.b5(l5)
        l5 = self.act(l5)

        # conv layer 6 [bs, 2, 2, 512]
        l6 = self.downsample(l5)
        l6 = self.conv6(l6)
        l6 = self.b6(l6)
        l6 = self.act(l6)
       
        # dense layer 7 [bs, 512]
        l7 = self.downsample(l6)
        l7 = torch.reshape(l7, [-1, 512])
        l7 = self.fc(l7)
        l7 = self.b7(l7)
        l7 = self.act(l7)

        # output layer (dense linear)
        logits = self.fc(l7)
        logits = self.b8(logits)

        return logits, [l0, l1, l2, l3, l4, l5, l6]



    
def main():
   sampler = Sampler()
   #print(list(sampler.parameters()))

if __name__ == '__main__':
    main()